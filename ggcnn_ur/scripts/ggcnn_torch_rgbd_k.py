...
MODEL_FILE = './output/models/210219_1340_newnet2/epoch_06_iou_0.80'  # new network2 DO-Conv+more filters
# MODEL_FILE = './output/newnetwork0216/epoch_07_iou_0.87' # new network DO-Conv
# worked for old obj set(before borrow from r)'./output/models/210210_1405_2heightsquare/epoch_07_iou_1.00' 
# worked for objects before new year   #'./output/models/210211_1356_all_k/epoch_13_iou_0.87' #new and old objects
...
def process_depth_image(depth, RGB, crop_size, out_size=300, return_mask=False, crop_y_offset=0):
    ...
    # fack_bg_rgb = (151.0, 127.0, 100.0)  # a fixed value to make all imgs consistent
    # RGB_crop[0:40,:] = fack_bg_rgb
    # RGB_crop[265:300,:] = fack_bg_rgb
    # RGB_crop[:,0:50] = fack_bg_rgb
    # RGB_crop[:,600:640] = fack_bg_rgb
    ...
    # fack_bg_depth = 1058/1000.0 #887.0 for box # a fixed value to make all depth imgs consistent
    # depth_crop[0:40,:] = fack_bg_depth
    # depth_crop[265:300,:] = fack_bg_depth
    # depth_crop[:,0:50] = fack_bg_depth
    # depth_crop[:,600:640] = fack_bg_depth
...
def post_processing(depth, depth_crop, RGB, points, angle, width_img, k_img, crop_size=300, crop_y_offset=0):
    ...
    g.k = k_img[best_g_unr] # predicted object height
    g.angle = ((angle[best_g_unr]%np.pi) - np.pi/2) /np.pi*180.0

    # modify detected z according to predicted height: !!
    # if g.k/2.0>8.0:
        # print("Tall object. Set z from ", g.pose.position.z," to ",1050.0 - (g.k/2.0)," or", g.pose.position.z)
        # g.pose.position.z = min(g.pose.position.z, 1050.0 - (g.k/2.0))
    # else:
        # print("Flat object. Set z from ", g.pose.position.z," to ",1050.0)
        # g.pose.position.z = 1050.0  #table
    # g.pose.position.z = 1050.0  #test (all set to 1050)
    return g
