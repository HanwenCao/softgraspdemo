...
from models.common_k import post_process_output#
from utils.dataset_processing import evaluation_k as evaluation#
from utils.dataset_processing import grasp_k as grasp#
    ...
    with torch.no_grad():
      if args.dataset=="my" or args.dataset=="my2" or args.dataset=="my_crop":
        for idx, (x, didx, rot, zoom) in enumerate(test_data):
            ...
            _pos_output, _cos_output, _sin_output, _width_output, _k_output = net.forward(xc) # only x(data) is used
            tq_img, tang_img, twidth_img, tk_img = post_process_output(_pos_output,_cos_output,_sin_output,_width_output, _k_output)
            if args.vis:
        ...
        for idx, (x, y, didx, rot, zoom) in enumerate(test_data):
            ...
            q_img, ang_img, width_img, k_img = post_process_output(lossd['pred']['pos'], lossd['pred']['cos'],
                                                        lossd['pred']['sin'], lossd['pred']['width'], lossd['pred']['k'])
            ...
            if args.vis:
                evaluation.plot_output(test_data.dataset.get_rgb(didx, rot, zoom, normalise=False),
                                       ...
                                       ang_img, no_grasps=args.n_grasps, grasp_width_img=width_img, k_img=k_img)
