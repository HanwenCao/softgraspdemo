...
def _gr_text_to_no(l, offset=(0, 0)):
    ...
    """
    x, y, k = l.split()
    return [int(round(float(y))) - offset[0], int(round(float(x))) - offset[1], int(round(float(k)))]
...
class GraspRectangles:
    ...
    def load_from_array(cls, arr):
        ...
        :param arr: Nx4x3 array, where each 4x3 array is the 4 corner pixels and k of a grasp rectangle.
    ...
    def load_from_cornell_file(cls, fname):
        ...
        with open(fname) as f:
            while True:
                # Load 4 lines at a time, corners of bounding box. yx+k
                p0 = f.readline()
                if not p0:
                    break  # EOF
    ...
    def draw(self, shape, position=True, angle=True, width=True, k=True):
        """
        ...
        return pos_out, ang_out, width_out, k_out
    ...
    def to_array(self, pad_to=0):
        ...
        if pad_to:
           if pad_to > len(self.grs):
               ...
               a = np.concatenate((a, np.zeros((pad_to - len(self.grs), 4, 3))))
    ...
    def center(self):
        ...
        points = [gr.pure_points for gr in self.grs]
        #print('1',points)
...
class GraspRectangle:
    ...
    def __init__(self, points):
        self.points = points # y x k
        #print('self.points',points)
        self.pure_points = points[:,0:2] #y x
        #print(self.pure_points)
        self.kk = points[:,2] #k
    ...
    def angle(self):
        ...
        """
        dx = self.pure_points[1, 1] - self.pure_points[0, 1]
        dy = self.pure_points[1, 0] - self.pure_points[0, 0]
        return (np.arctan2(-dy, dx) + np.pi/2) % np.pi - np.pi/2

    @property
    def k(self):
        """
        :return: k of the grasp.
        """
        #k = self.points[:,2]
        #k = k.reshape((-1,1))
        #print('k=',self.kk)
    ...
    def center(self):
        ...
        """
        #pure_points = self.points[:,0:2]
        return self.pure_points.mean(axis=0).astype(np.int)
    ...
    def width(self):
        ...
        """
        dy = self.pure_points[2, 1] - self.pure_points[1, 1]
        dx = self.pure_points[2, 0] - self.pure_points[1, 0]
        return np.sqrt(dx ** 2 + dy ** 2)
    ...
    def polygon_coords(self, shape=None):
        ...
        """
        return polygon(self.pure_points[:, 0], self.pure_points[:, 1], shape)
    ...
    def iou(self, gr, angle_threshold=np.pi/6):
        ...
        rr2, cc2 = polygon(gr.pure_points[:, 0], gr.pure_points[:, 1])
    ...
    def offset(self, offset):
        ...
        """
        #pure_points = self.points[:,0:2]
        self.pure_points += np.array(offset).reshape((1, 2))
    ...
    def rotate(self, angle, center):
        ...
        self.pure_points = ((np.dot(R, (self.pure_points - c).T)).T + c).astype(np.int)
    ...
    def plot(self, ax, color=None):
        ...
        """
        points = np.vstack((self.pure_points, self.pure_points[0]))
    ...
    def zoom(self, factor, center):
        ...
        c = np.array(center).reshape((1, 2))
        self.pure_points = ((np.dot(T, (self.pure_points - c).T)).T + c).astype(np.int)
...
class Grasp:
    ...
    def as_gr(self):
        ...
        return GraspRectangle(np.array(
            [
             [y1 - self.width/2 * xo, x1 - self.width/2 * yo, k0],
             [y2 - self.width/2 * xo, x2 - self.width/2 * yo,k0],
             [y2 + self.width/2 * xo, x2 + self.width/2 * yo,k0],
             [y1 + self.width/2 * xo, x1 + self.width/2 * yo,k0],
             ]#  #fake k value
        ).astype(np.float))
