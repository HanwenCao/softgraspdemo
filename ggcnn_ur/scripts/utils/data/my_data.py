...
class MyDataset(GraspDatasetBase):
    """
    ...
    def __init__(self, file_path, start=0.0, end=1.0, ds_rotate=0, **kwargs):
        ...
        graspf = glob.glob(os.path.join(file_path, '*_RGB.png'))  #if images are directly under the path
        graspf.sort()
        l = len(graspf)
        ...
        depthf = [f.replace('RGB.png', 'perfect_depth.tiff') for f in graspf]
