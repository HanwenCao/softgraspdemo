...
def detect(save_img=False):
    ...
    # inpaint by position
    fack_bg_depth = np.median(depth_arr_crop[250-5:250+5,485-15:485+5])
    print('fack_bg_depth',fack_bg_depth)
    fack_bg_depth = 1052 #887.0 for box # a fixed value to make all depth imgs consistent
    depth_arr_crop[0:60,:] = fack_bg_depth
    depth_arr_crop[350:480,:] = fack_bg_depth
    depth_arr_crop[:,0:50] = fack_bg_depth
    depth_arr_crop[:,600:640] = fack_bg_depth
    ...
    # inpaint by position-RGB
    fack_bg_rgb = (np.median(img_arr_crop[250-5:250+5,485-5:485+5,0]),np.median(img_arr_crop[358-5:358+5,485-5:485+5,1]),np.median(img_arr_crop[358-5:358+5,485-5:485+5,2]))
    print('fack_bg_rgb',fack_bg_rgb)
    fack_bg_rgb = (138.0, 127.0, 114.0)  # a fixed value to make all imgs consistent
    img_arr_crop[0:60,:] = fack_bg_rgb
    img_arr_crop[350:480,:] = fack_bg_rgb
    img_arr_crop[:,0:50] = fack_bg_rgb
    img_arr_crop[:,600:640] = fack_bg_rgb
    ...
    depth_arr_crop = depth_arr_crop/1000.0  # mm -> m
    ...
    # depth_vis = plt.imshow(depth_read_tiff)#
    # plt.axis('off')

    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_subplot(1,1, 1)
    plot = ax.imshow(depth_read_tiff, cmap='turbo', vmin=0.97, vmax=1.10)
    ax.axis('off')
    plt.colorbar(plot)
    ...
    if view_img:
    	...
    	# vis Depth
    	# depth_int = ((depth_read_tiff/np.max(depth_read_tiff))*255.0).astype(int)
    	# depth_int = np.uint8(depth_int)
    	# depth_int = cv2.normalize(depth_int,  depth_int, 0, 255, cv2.NORM_MINMAX)
