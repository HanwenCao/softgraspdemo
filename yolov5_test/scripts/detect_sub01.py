...
def detect(save_img=False):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    conf_list.append(conf_numpy)
                max_conf=max(conf_list)
                # Write results
                
                for *xyxy, conf, cls in reversed(det):
                    conf_numpy1=conf.cpu().numpy()
                    #estimate max confidence          
                    if conf_numpy1==max_conf:
                      # average depth
                      ...
                      x1cc, x2cc, y1cc, y2cc = (x1c+x1)/2,(x2c+x2)/2,(y1c+y1)/2,(y2c+y2)/2
                      depth_8samples = [depth_arr[int(y1c),int(x1c)],depth_arr[int(y1c),int(x2c)],depth_arr[int(y2c),int(x1c)],depth_arr[int(y2c),int(x2c)],depth_arr[int(y1cc),int(x1cc)],depth_arr[int(y1cc),int(x2cc)],depth_arr[int(y2cc),int(x1cc)],depth_arr[int(y2cc),int(x2cc)]]
                      #print('depth_8samples:',depth_4samples)
                      depth_validsamples = [b for b in depth_8samples if b>0]
                      #print(depth_validsamples)
                      depth_max = max(depth_validsamples)
                      depth_min = min(depth_validsamples)
                      depth_validsamples.remove(depth_max)
                      depth_validsamples.remove(depth_min)
                      #print(depth_validsamples)
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        xc,yc = (x1+x2)/2, (y1+y2)/2
                        x1c, x2c, y1c, y2c = (x1+xc)/2,(x2+xc)/2,(y1+yc)/2,(y2+yc)/2
                        x1cc, x2cc, y1cc, y2cc = (x1c+xc)/2,(x2c+xc)/2,(y1c+yc)/2,(y2c+yc)/2
                        depth_8samples = [depth_arr[int(y1c),int(x1c)],depth_arr[int(y1c),int(x2c)],depth_arr[int(y2c),int(x1c)],depth_arr[int(y2c),int(x2c)],depth_arr[int(y1cc),int(x1cc)],depth_arr[int(y1cc),int(x2cc)],depth_arr[int(y2cc),int(x1cc)],depth_arr[int(y2cc),int(x2cc)]]
                        #print('depth_8samples:',depth_4samples)
                        depth_validsamples = [b for b in depth_8samples if b>0]
                        depth_max = max(depth_validsamples)
    ...
    return 0,0,0,0,xyz_obj[0],xyz_obj[1],xyz_obj[2],clas
    ...
    while True:
        ...
        with torch.no_grad():
            if opt.update:  # update all models (to fix SourceChangeWarning)
                ...
                im_pil.show()'''
                yolo_results = detect()
                print('yolo_results =', yolo_results[4:8])            
